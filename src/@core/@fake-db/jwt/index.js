...
mock.onPost('/jwt/register').reply(request => {
  ...
  const error = {
    password: !password ? ['Introduza a palavra-passe'] : null,
    email: (() => {
      if (!email) return ['Introduza o seu e-mail.']
      if (isEmailAlreadyInUse) return ['Este e-mail já está em uso.']
      return null
    })(),
    username: (() => {
      if (!username) return ['Introduza o seu nome de utilizador.']
      if (isUsernameAlreadyInUse) return ['Esse nome de usuário já está em uso.']
      return null
    })(),
  }
